...
/** 
 * Add a small interface for the cached data that has candidateIds.  
 * (Or adjust to match the shape that's actually stored in the query cache.)
 */
...
const SearchResults = () => {
  ...
  const { data: searchResults } = useQuery<SearchResultsCacheData>({
    queryKey: ['searchResults'],
    queryFn: () => {
      const data = queryClient.getQueryData(['searchResults']) as SearchResultsCacheData;
      return data || { results: { candidateIds: [] } };
    },
    enabled: true,
  ...
  const selectedCandidateIds = shortlistedCandidates
    .map(c => c.profile?.public_identifier)
  ...
  const handleProfileClick = (public_identifier: string) => {
    ...
    const candidate = candidates.results.find((c: SearchResult) => {
